...
export async function POST(request: Request, { params }: SendEmailRouteProps) {
  ...
  const safeObject = job.vehicle ? escapeHtml(job.vehicle) : "";
  const safeReference = job.licensePlate ? escapeHtml(job.licensePlate) : "";

  const objectLine =
    safeObject || safeReference
      ? `${safeObject}${safeObject && safeReference ? " · " : ""}${
          safeReference ? `Referenz: ${safeReference}` : ""
        }`
      : "";

  const plainObjectLine =
  ...
  const subject = `${company.name}: Freigabe für Zusatzleistung erforderlich`;
      ...
      <div style="max-width:640px; margin:0 auto; padding:32px 16px; font-family:Arial, sans-serif; color:#0f172a;">
        <div style="background:#ffffff; border:1px solid #e2e8f0; border-radius:18px; padding:28px;">
          ...
          <h1 style="margin:0; font-size:26px; line-height:1.25; color:#0f172a;">
            Bitte Zusatzleistung prüfen und freigeben
          </h1>
          ...
          <p style="margin:12px 0 0; font-size:16px; line-height:1.7;">
            ${safeCompanyName} bittet Sie um eine Online-Freigabe für eine zusätzliche Leistung oder einen Nachtrag zu Ihrem Auftrag.
          </p>
          ...
          <div style="margin:24px 0 0; padding:18px; border-radius:14px; background:#f1f5f9;">
            ...
            ${
              objectLine
                ? `<p style="margin:8px 0 0; color:#475569; font-size:14px;">${objectLine}</p>`
                : ""
            }
          </div>

          <div style="margin:16px 0 0; padding:18px; border:1px solid #e2e8f0; border-radius:14px;">
            <p style="margin:0 0 6px; color:#64748b; font-size:13px;">Zusatzleistung / Nachtrag</p>
            ...
            ${
              safePriceText
                ? `<p style="margin:14px 0 0; font-size:16px; font-weight:700;">Preis / Kostenhinweis: ${safePriceText}</p>`
                : ""
            }
          </div>
          ...
          <div style="margin:24px 0 0; padding:16px; border-radius:14px; background:#fffbeb; color:#78350f; font-size:13px; line-height:1.6;">
            Bei Fragen zur Leistung, zum Preis oder zum Auftrag können Sie direkt auf diese E-Mail antworten.
  ...
  const text = [
    ...
    `Hallo ${customer.name},`,
    "",
    `${company.name} bittet Sie um eine Online-Freigabe für eine zusätzliche Leistung oder einen Nachtrag zu Ihrem Auftrag.`,
    "",
    `Auftrag: ${job.title}`,
    plainObjectLine ? `Einsatzort / Referenz: ${plainObjectLine}` : "",
    "",
    `Zusatzleistung / Nachtrag: ${item.title}`,
    item.description,
    item.priceText ? `Preis / Kostenhinweis: ${item.priceText}` : "",
    "",
    `Freigabe öffnen: ${approvalUrl}`,
    "",
    `Bei Fragen zur Leistung, zum Preis oder zum Auftrag können Sie direkt auf diese E-Mail antworten. Ihre Antwort geht an ${company.name}.`,
    "",
    `Gesendet mit ${APP_NAME}.`,
  ]
    .filter(Boolean)
